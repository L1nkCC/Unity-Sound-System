...
namespace CC.SoundSystem.Editor
{
    public static class DomainNoticeHandler
    {
        ...
        public static void AddDomainNotices(this VisualElement parent, string domainName)
        {
            foreach(Notice notice in m_notices)
            {
                UpdateErrorStatus(parent, domainName, notice);
            }
        }


        private static void UpdateErrorStatus(VisualElement parent, string domainName, Notice notice)
        {
